...
regions <- fromJSON(file = jsonfile, unexpected.escape="keep")
#print(regions)
# Convert JSON file to a data frame.
region_list <- as.list(regions)

# voter file
voterreg_data_path <- paste("/project/fehder_718/raw_l2/inter/", year ,"/l2_region_files/",  sep="") # nolint: line_length_linter.
...
#exec data
exec_data_path <- paste("/project/fehder_718/raw_inventors_by_year/inter/by_year_w_imputation/", year, "/",sep="")
exec_file <- paste("researcher_region_", region_list[[rindex]], ".csv", sep="")
exec_file_path <- paste(exec_data_path, exec_file, sep="")

# load data
voterRegData <- read_csv(voter_file_path)

# specify import types
col_spec <- list(inventor_id = "c", location_id = "c", name_first_base = "c", name_last_base = "c",  # nolint: line_length_linter.
    male_flag = "d", city = "c", state = "c", region_code = "c", name_suffix = "c", # nolint: indentation_linter.
    name_middle_initial = "c", name_first = "c", name_last = "c", year = "d",
    gender = "d")

execData <- read_csv(exec_file_path, col_types = col_spec)
...
#voterRegData <- subset(voterRegData, hhincome > 70000)
...
#     varnames = c("name_first", "name_last", "name_middle_initial", "name_suffix", "gender", "dTime"),
...
ff <- fastLink( 
    execData, voterRegData,
    varnames = c("name_first", "name_last", "name_middle_initial", "gender"),
    stringdist.match = c("name_first", "name_last"),
    partial.match = c("name_first", "name_last"),
    threshold.match = .02,
    priors.obj = list(lambda.prior=lam),
...
gamvec <- c("gamma.1", "gamma.2", "gamma.3", "gamma.4")
...
out_path <- paste("/project/fehder_718/ftrl_2024_respol_data/final/matches/", year, "/", sep="")
